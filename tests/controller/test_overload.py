...
def test_kv_usage_triggers_overload():
    ...
    od.observe(kv_usage_pct=85.0, preempts_delta=0, queue_depth=0)
    assert not od.overloaded
    od.observe(kv_usage_pct=91.0, preempts_delta=0, queue_depth=0)
...
def test_preempt_streak_triggers_overload():
    ...
    for _ in range(2):
        od.observe(kv_usage_pct=50.0, preempts_delta=1, queue_depth=0)
    assert not od.overloaded
    od.observe(kv_usage_pct=50.0, preempts_delta=1, queue_depth=0)
    assert od.overloaded
...
def test_preempt_streak_resets_on_zero():
    ...
    od.observe(kv_usage_pct=50.0, preempts_delta=1, queue_depth=0)
    od.observe(kv_usage_pct=50.0, preempts_delta=1, queue_depth=0)
    od.observe(kv_usage_pct=50.0, preempts_delta=0, queue_depth=0)  # reset
    od.observe(kv_usage_pct=50.0, preempts_delta=1, queue_depth=0)
    assert not od.overloaded
...
def test_queue_depth_threshold_optional():
    ...
    od.observe(kv_usage_pct=50.0, preempts_delta=0, queue_depth=50)
    assert not od.overloaded
    od.observe(kv_usage_pct=50.0, preempts_delta=0, queue_depth=200)
    assert od.overloaded
